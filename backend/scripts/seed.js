...
const Messages = () => {
  ...
  const selectedKey = activeConversation
    ? `${activeConversation.bookingId}:${activeConversation.staffRole}`
    : "";
  ...
  useEffect(() => {
    ...
    const syncConversationMeta = (nextMessages, serverConversation = null) => {
      const latestMessage =
          ...
          .map((conversation) => {
            const isSameConversation =
              String(conversation.bookingId) ===
                String(activeConversation.bookingId) &&
              conversation.staffRole === activeConversation.staffRole;
            ...
            return {
              ...conversation,
              ...(serverConversation || {}),
              lastMessage: latestMessage || conversation.lastMessage || null,
              updatedAt:
      ...
      setActiveConversation((current) => {
        ...
        const isSameConversation =
          String(current.bookingId) === String(activeConversation.bookingId) &&
          current.staffRole === activeConversation.staffRole;

        if (!isSameConversation) return current;
    ...
    const loadMessages = async (silent = false) => {
      ...
      try {
        ...
        syncConversationMeta(nextMessages, res.conversation || null);
      } catch (err) {
        if (!isMounted) return;
  ...
  }, [activeConversation, isTabVisible]);
