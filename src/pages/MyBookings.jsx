...
import { getAllBookings, deleteBooking, getEsewaSignature } from '../api/bookingApi';
        ...
        <div className="min-h-screen p-10 bg-gray-100">
            ...
            <div className="grid gap-6">
                {bookings.map((booking) => (
                    <div 
                        key={booking._id} 
                        onClick={() => navigate(`/tour/${booking.destinationId?._id}`)}
                    ...
                    >
                        <div>
                            <h3 className="font-bold text-xl text-[#004d4d]">{booking.destinationId?.title}</h3>
                            <p className="text-sm text-gray-600">Total: Rs. {booking.totalPrice}</p>
                            <span className={`text-xs font-bold uppercase ${booking.status === 'confirmed' ? 'text-green-600' : 'text-orange-500'}`}>
                                {booking.status}
                            </span>
                        </div>
