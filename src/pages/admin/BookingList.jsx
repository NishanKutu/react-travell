...
import { getAllBookings, deleteBooking, cancelAndRefund } from "../../api/bookingApi";
  ...
  const handleDelete = async (booking) => {
    ...
    if (window.confirm(confirmMessage)) {
      try {
        let response;
        if (isConfirmed) {
          // If confirmed, call the refund logic instead of deleting
          response = await cancelAndRefund(booking._id);
        } else {
          // If pending or already cancelled, proceed with deletion
          response = await deleteBooking(booking._id);
        }

        if (response.success) {
          toast.success(response.message);
          // Refresh the list to show updated status or removed item
          fetchBookings();
        } else {
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-slate-500 text-sm font-medium">
              Manage trekking schedules and guide assignments.
            </p>
          </div>
            ...
            <table className="min-w-full leading-normal">
              <thead>
                <tr className="bg-slate-50">
                  ...
                  <th className="px-5 py-4 border-b border-gray-200 text-left text-[11px] font-black text-slate-400 uppercase tracking-widest text-center">
                    Travelers
                  </th>
                  <th className="px-5 py-4 border-b border-gray-200 text-left text-[11px] font-black text-slate-400 uppercase tracking-widest">
                    Selected Guide & Schedule
                  </th>
                  <th className="px-5 py-4 border-b border-gray-200 text-left text-[11px] font-black text-slate-400 uppercase tracking-widest">
                    Total Amount
                  </th>
                      ...
                      {booking.hasGuide && booking.guideId ? (
                        <div className="space-y-2">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-slate-800 font-bold text-xs leading-none">
                                {booking.guideId.username}
                              </p>
                              <p className="text-emerald-500 font-black text-[9px] uppercase tracking-tighter">
                                Verified Guide
                              </p>
                            </div>
                          </div>
